...
- Notebook 15 uses scalar codebooks by default because this makes `num_bits`
...
- It is not the default in Notebook 15 because an `n`-bit assignment for a
...
Notebook 15 sets `d = 1`, so `n` is the actual index width per quantized
...
runtime. Activations remain FP32, and no fake-quantization nodes are inserted.
