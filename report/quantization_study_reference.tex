...
\title{Analysis of Quantization Methods in Transformer-Based Neural Networks}
...
\begin{abstract}
Transformer models achieve strong results in language and vision tasks, but their parameter counts and memory demands complicate efficient deployment. This group study analyzes weight quantization in two transformer architectures---DistilBERT and ViT-B/16---with ResNet-18 as a CNN reference. It compares symmetric per-channel post-training quantization (PTQ) with tensor-local scalar codebooks, using floating-point reconstruction with FP32 activations and no quantization-aware training. Experiments use the complete CIFAR-10 test split and labelled SST-2 development split. Two-to-eight-bit sweeps measure accuracy and estimate parameter compression, while four- and eight-bit layer or module studies measure isolated sensitivity and cross-bit rank stability. At eight bits, the maximum accuracy degradation across all three models and both custom methods is 0.59 percentage points, with approximately 75\% parameter-storage reduction. At four bits, DistilBERT and ViT remain within 0.6 percentage points of their FP32 baselines while achieving about $7.9\times$ compression. ResNet-18 instead falls from 90.47\% to 11.56\% with PTQ and 8.43\% with codebook quantization; quantizing its first convolution alone causes losses of 60.65 and 78.62 percentage points. These observations concern the evaluated checkpoints and pipeline. ViT used a frozen backbone whereas ResNet was adapted end-to-end; training and checkpoint differences prevent isolating an architecture effect. The cause of the severe ResNet degradation remains unresolved.
\end{abstract}
...
\begin{enumerate}
    ...
    \item How do the selected DistilBERT, ViT-B/16 and ResNet-18 checkpoints respond to four- and eight-bit quantization under the evaluated policies, given their different training procedures?
...
\begin{itemize}
    ...
    \item a common quantization and evaluation framework applied to three separately trained checkpoints;
    ...
    \item a checkpoint-specific comparison that documents measured accuracy changes and the limits of attributing them to architecture.
\end{itemize}
...
ViT is evaluated on the same 10,000 CIFAR-10 test images and labels as ResNet-18, using its own preset-specific resizing and normalization. The FP32 checkpoint correctly classifies 9,573 images, giving 95.73\% accuracy. This is a comparison of two selected checkpoints on a shared dataset, not a matched-training architecture experiment. ViT retained its pretrained backbone while ResNet was adapted end-to-end. Consequently, an observed difference in quantization response may involve adaptation, checkpoint quality and architecture; this design does not separate these effects.
...
The methods are described as \emph{research-informed custom implementations}. Their software implementation and experimental integration are project work, but their mathematical foundations come from prior studies. Jacob et al. establish affine integer quantization and integer accumulation \cite{jacob2018}; Krishnamoorthi discusses post-training calibration and per-channel weight quantization \cite{krishnamoorthi2018}; and Nagel et al. provide a general PTQ framework and analysis of low-bit failure modes \cite{nagel2021}. The codebook path draws from classical vector quantization \cite{gray1984}, product quantization \cite{jegou2011}, neural-network vector quantization \cite{gong2014}, weight sharing \cite{han2016}, and Hessian-weighted distortion \cite{choi2017}. Optional library functions are additionally informed by Diffusion Product Quantization (DPQ) \cite{shao2024} and VQ4ALL \cite{deng2024}. The principal experiments, however, use tensor-local scalar codebooks rather than the complete training procedures proposed by DPQ or VQ4ALL.

\subsection{Transformer Quantization Context}

Transformer PTQ literature distinguishes low-bit weight storage from quantized activation arithmetic. GPTQ uses approximate second-order information for weight quantization of large generative transformers \cite{frantar2022}. AWQ uses activation statistics to identify and scale salient weight channels for low-bit weight-only quantization \cite{lin2023}. These methods study larger language models and different evaluation tasks from the DistilBERT sentiment classifier used here. Their results provide context, not directly comparable baselines; neither method was implemented or benchmarked in this study.

SmoothQuant addresses activation outliers through an equivalent rescaling of weights and activations for W8A8 inference \cite{xiao2022}. LLM.int8() handles outlier feature dimensions with a higher-precision matrix multiplication alongside INT8 operations \cite{dettmers2022}. For vision transformers, PTQ4ViT introduces twin uniform quantization for activation distributions and Hessian-guided calibration, evaluated on ImageNet \cite{yuan2021}. These studies show why weight-only results with FP32 activations do not establish robustness to activation quantization. The present CIFAR-10 and SST-2 experiments evaluate simpler weight quantizers, and do not claim a new transformer quantization method or reproduce those deployment results.
...
\label{eq:custom-ptq}
\end{equation}
The last tensor axis is treated as the output-channel axis. If a channel is all zero, a safe nonzero scale is used and its reconstruction remains zero. The implementation retains quantized values in NumPy INT8 arrays and FP32 channel scales. Reported storage estimates assume bit-packed values and include scale metadata and preserved FP32 parameters. The symmetric grid uses $2^b-1$ levels, whereas a $b$-bit codebook uses $2^b$ centroids.

The codebase also supports representative-data activation-range calibration, which is not used in the custom accuracy sweeps reported here. The evaluated custom models reconstruct the low-bit weights into floating arrays and execute with FP32 activations in Keras.
...
\label{eq:codebook}
\end{equation}
The centroids are fitted with fixed-seed k-means++ initialization \cite{arthur2007} followed by Lloyd updates \cite{lloyd1982}. The evaluated notebooks fit each tensor on all scalar values up to 100,000, or on a uniform sample of 100,000 without replacement for larger tensors. Exact nearest-centroid assignment is then performed for every selected value. Table~\ref{tab:codebook-config} specifies the recorded notebook settings and implementation defaults. A $b$-bit experiment uses $K=2^b$ centroids, requiring $b$ assignment bits.
...
\begin{table}[!t]
\caption{Scalar-codebook configuration in the evaluated notebooks. The iteration cap is the notebook override; tolerance and initialization count follow the custom NumPy fitter.}
\label{tab:codebook-config}
\centering
\scriptsize
...
\midrule
Grouping & One scalar codebook per selected tensor; $d=1$ \\
Centroids & $K=2^b$; FP32 centroids and fitting data \\
Fitting sample & All values if $N\leq100{,}000$; otherwise 100,000 sampled without replacement \\
Initialization & Fixed-seed k-means++; one initialization, no multiple restarts \\
Lloyd iterations & At most 30 per fit \\
Stopping rule & Maximum centroid displacement at most 0.000001 in weight units \\
Seed policy & Base seed 42 plus the zero-based index in the complete \texttt{model.weights} list \\
Random sampling & Sampling and initialization each use a NumPy generator with that tensor seed \\
Empty clusters & Refilled from the current farthest fitting samples \\
Final assignment & Nearest stored centroid for every selected scalar value \\
\bottomrule
\end{tabular}
\end{table}

The two methods differ in more than grid placement. PTQ fits a separate uniform scale for each last-axis channel and uses $2^b-1$ symmetric levels. Codebook quantization fits one non-uniform codebook per tensor with $2^b$ centroids. Granularity and level count therefore confound any attribution to uniform versus non-uniform placement. The measured comparisons apply to these complete configurations; a matched-granularity, matched-level control was not evaluated.
...
The recorded custom ResNet runs leave BatchNorm layers in the model and retain their saved FP32 scale, offset, moving mean and moving variance. Inference calls use \texttt{training=False}. No BatchNorm folding, post-quantization statistics re-estimation, bias correction, cross-layer equalization or adaptive rounding is performed in these runs. This describes the evaluated policy; it does not establish whether a different policy would recover W4 accuracy.
...
Compression and memory reduction contain the same storage information: $S=100(1-1/R)$ before rounding. Figure~\ref{fig:all-model-tradeoffs} therefore plots only accuracy and compression; the tables retain $S$ as an alternative expression of storage savings.

\subsection{FP32 Full-Model Evaluation}

The first experiment establishes the FP32 prediction baseline for each architecture. ResNet-18 and ViT-B/16 are evaluated on all 10,000 images in the official CIFAR-10 test split. DistilBERT is evaluated on all 872 labelled SST-2 development examples. Each checkpoint is evaluated without modifying its parameters and without test-time augmentation. Table~\ref{tab:model-dataset-summary} consolidates the model properties, evaluation sizes and FP32 reference accuracies; the corresponding correct-prediction counts are stated in Section~\ref{sec:models-datasets}. All later accuracy differences, compression ratios, and memory reductions use the corresponding FP32 model as their baseline.
...
Table~\ref{tab:w8-results} shows small observed W8 accuracy changes for the three evaluated checkpoints. Across the custom methods, the largest accuracy loss is 0.59 percentage points for ResNet PTQ. All custom W8 representations are approximately $4\times$ smaller than FP32 and reduce parameter storage by about 75\%.
...
\caption{Original plots from Notebook~25: bit-wise accuracy across all three architectures for PTQ (left) and codebook quantization (right). Dashed lines show the corresponding FP32 baselines.}
\label{fig:cross-model-bitwise-accuracy}
\end{figure*}

ResNet-18 exhibits a severe low-bit transition under both methods (Table~\ref{tab:resnet-sweep}). PTQ remains near chance from two through four bits, then rises to 49.39\% at five bits, 71.87\% at six bits, and 88.05\% at seven bits. Codebook quantization recovers later: it reaches only 48.71\% at six bits but rises to 88.97\% at seven bits and 90.25\% at eight bits. Thus, W8 codebook is slightly closer to FP32 than W8 PTQ, but both evaluated configurations exhibit severe full-model W4 degradation on this checkpoint. The cause remains unresolved; these measurements do not establish a general limitation of ResNet or CNN quantization. Nagel et al. report much stronger W4 ResNet results on ImageNet using a more elaborate PTQ pipeline, including adaptive rounding \cite{nagel2021}. Different checkpoints, datasets and quantization procedures prevent a direct numerical comparison, and the present study does not identify which differences explain the gap.
...
Across the completed sweeps, compression is governed mainly by bit width because the selected tensors contain most model parameters. Two-bit representations provide approximately $15.5$--$15.8\times$ compression and about 93.6\% parameter-memory reduction. Four-bit representations provide approximately $7.9\times$ compression and about 87.3\% reduction. Eight-bit representations provide approximately $4\times$ compression and about 74.8--74.9\% reduction. Observed accuracy varies strongly across the evaluated checkpoints and quantization configurations; the experiment does not isolate architecture as its cause.

Figure~\ref{fig:all-model-tradeoffs} consolidates accuracy and estimated compression for all 42 configurations. The near-overlap of compression curves follows mainly from the shared bit widths and parameter-count accounting; it is not an independent finding about quantization quality. The accuracy curves show different checkpoint- and configuration-specific responses.

\begin{figure*}[!t]
\centering
\includegraphics[width=.98\textwidth]{all_models_accuracy_compression_2_to_8_bit.png}
\caption{Consolidated two-to-eight-bit results from Notebook~25. Rows correspond to ResNet-18, DistilBERT, and ViT-B/16; columns show accuracy and estimated parameter compression. Memory reduction is omitted because $S=100(1-1/R)$ and is retained in the tables. Dashed lines indicate the checkpoint-specific FP32 accuracies.}
...
The sensitivity experiments identify vulnerable components and test whether their W8 ranking transfers to W4. Each isolated run starts from the original FP32 checkpoint, quantizes exactly one analysis unit, leaves all other parameters FP32, and evaluates the complete labelled split. ResNet uses individual kernels, DistilBERT individual embedding or kernel tensors, and ViT groups 76 eligible tensors into 14 logical modules. Accuracy degradation is
\begin{equation}
\Delta A_u=A_{\mathrm{FP32}}-A_u,
\end{equation}
where $A_u$ is accuracy with only unit $u$ quantized. Negative degradation denotes an observed accuracy increase, not evidence of improved generalization. Units are ranked within each method and precision by descending degradation, breaking ties by descending prediction disagreement (the percentage of examples whose predicted class changes).

Table~\ref{tab:sensitivity-coverage} gives the coverage per method. ResNet is exhaustive at both precisions. The transformer W4 subsets were fixed by architectural role and depth before inspecting W4 outcomes, independently of W8 sensitivity; they are neither random samples nor selections of the least-sensitive units. DistilBERT includes both embeddings; query, attention-output and feed-forward-output kernels from blocks 0 and 5; query, value, attention-output and both feed-forward kernels from block 2; and the pooled and classifier kernels ($2+3+3+5+2=15$). ViT includes input embedding, blocks 1, 3, 6, 9 and 11, and the classifier. All block indices are zero-based.

Statistical significance uses the exact conditional, two-sided binomial form of McNemar's test for paired FP32/quantized correctness \cite{fagerland2013}. Let $n_{cw}$ and $n_{wc}$ count examples that change from correct to wrong and wrong to correct. Conditional on $n=n_{cw}+n_{wc}$, the null distribution is $\operatorname{Binomial}(n,1/2)$. The notebooks use \texttt{scipy.stats.binomtest} with its two-sided alternative, assigning $p=1$ when $n=0$. These are binomial-tail $p$-values, not the chi-square approximation. The exact conditional test can be conservative with few discordant pairs \cite{fagerland2013}. Benjamini--Hochberg correction is applied across the tested units separately within each method and precision, with adjusted $p<0.05$ defining a significant change \cite{benjamini1995}. The sign of $\Delta A_u$ distinguishes degradation from improvement.

The discreteness of this test limits what the transformer experiments can establish. For $n>0$ discordant examples, the smallest attainable two-sided exact value is $p_{\min}=2^{1-n}$; when $n=0$, the assigned value is one. Table~\ref{tab:mcnemar-resolution} applies this arithmetic to the recorded DistilBERT outcomes. ``Raw impossible'' counts units for which $p<0.05$ cannot be attained even if every discordance has the same direction. ``First BH impossible'' uses the stricter $0.05/m$ cutoff for $m$ tests; it describes whether a unit could occupy the first rejection position by itself, not an absolute impossibility under every multi-rejection BH configuration. The absence of rejected hypotheses therefore is not evidence of statistical equivalence or insensitivity.

\begin{table}[!t]
\centering
\caption{Isolated sensitivity coverage per quantization method. Transformer W4 units form predefined architectural subsets.}
...
DistilBERT & Tensor & 40/40 & 15/40 & 872 \\
ViT-B/16 & Module & 14/14 & 7/14 & 10,000 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!t]
\centering
\caption{Resolution of the exact McNemar tests for DistilBERT. Discordant counts show minimum--median--maximum.}
\label{tab:mcnemar-resolution}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}llrrrr@{}}
\toprule
Bits & Method & Tests & Discordant $n$ & Raw impossible & First BH impossible \\
\midrule
W8 & PTQ & 40 & 0--0--2 & 40 & 40 \\
W8 & Codebook & 40 & 0--0--12 & 39 & 39 \\
W4 & PTQ & 15 & 0--3--13 & 12 & 14 \\
W4 & Codebook & 15 & 0--2--9 & 12 & 15 \\
\bottomrule
\end{tabular}
\end{table}

% Figures are unmodified image files saved by Notebooks 20, 21 and 27.
\begin{figure*}[!t]
\centering
\includegraphics[width=.88\textwidth]{resnet18_w4_w8_layer_sensitivity.png}
\caption{ResNet-18 W4/W8 heatmap regenerated in Notebook~20 from saved measurements. The upper panel retains all 21 kernels and the symmetric $\pm80$-pp scale needed for the extreme losses. The lower panel omits the three largest PTQ W4 losses, repeats the remaining 18 kernels on a $\pm7$-pp scale, and prints their values. A dot marks an FDR-adjusted McNemar $p<0.05$. Both panels are zero-centered; red denotes loss and blue observed gain.}
\label{fig:resnet-sensitivity}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=.98\textwidth]{distilbert_w4_w8_layer_sensitivity.png}
\caption{DistilBERT matched W4/W8 heatmap regenerated in Notebook~21 from saved measurements for 15 of 40 tensors, in architectural selection order. Columns show PTQ W4, PTQ W8, codebook W4 and codebook W8. Zero is the neutral color midpoint; red denotes loss and blue observed gain. The symmetric $\pm0.35$-pp scale is shared with ViT and differs from ResNet's $\pm80$-pp scale.}
\label{fig:distilbert-sensitivity}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=.88\textwidth]{vit_w4_w8_module_sensitivity_heatmap.png}
\caption{ViT-B/16 W4/W8 heatmap generated in Notebook~27 from Notebook~22's saved matched measurements for seven of 14 modules, in architectural order. Columns show PTQ W4, PTQ W8, codebook W4 and codebook W8. Zero is the neutral color midpoint; red denotes loss and blue observed gain. The symmetric $\pm0.35$-pp scale is shared with DistilBERT and differs from the full and detail scales in the ResNet figure. All three plots use the same zero-centered color semantics; no inference was rerun.}
\label{fig:vit-sensitivity}
\end{figure*}

\subsubsection{Isolated W8 and W4 sensitivity}

Table~\ref{tab:isolated-sensitivity-summary} reports the maximum isolated loss. At W8, the largest loss is 0.53\pp{}. At W4, quantizing only ResNet's first convolution causes losses of 60.65\pp{} with PTQ and 78.62\pp{} with codebook quantization. Codebook produces 13 FDR-significant changes: 12 degradations and one improvement. The improvement is 0.41\pp{} for \texttt{stack1\_block1\_2\_conv/kernel} (adjusted $p=0.00274$). PTQ produces 14 significant degradations. No sampled DistilBERT or ViT unit passes its FDR threshold. For DistilBERT this null-rejection count is dominated by the exact test's resolution floor (Table~\ref{tab:mcnemar-resolution}) and must not be interpreted as evidence that the checkpoint is insensitive. The observed effect estimates remain small, but equivalence was not tested.

Figures~\ref{fig:resnet-sensitivity}--\ref{fig:vit-sensitivity} reproduce sensitivity heatmaps saved by Notebooks~20, 21 and 27; Notebook~27 visualizes the matched ViT measurements saved by Notebook~22. They use zero-centered colors. The ResNet figure preserves its full range and adds a detail panel so that the smaller significant changes remain legible; the transformers share a scale. Comparisons across different scales must use the numerical color bars. ResNet has severe early-kernel W4 losses, while the sampled transformer changes remain small in absolute pp. These isolated results identify vulnerable components under the evaluated policy, but do not establish how multiple quantization errors interact. Transformer figures show only matched W4/W8 subsets; the exhaustive W8 maxima in Table~\ref{tab:isolated-sensitivity-summary} may involve units outside those subsets.

\begin{table}[!t]
\centering
\caption{Maximum isolated degradation. W4 transformer results cover subsets. Sig.\ counts all FDR-significant changes, including improvements. Block indices are zero-based.}
...
Spearman correlations compare the assigned sensitivity ranks of matched units, recomputed within each matched subset. These ranks include the disagreement tie-break and therefore need not equal Spearman correlations of accuracy degradation alone with averaged ties.

Because only 21, 15 or 7 units are matched, the default approximate correlation $p$-values are validated by two-sided permutation tests of absolute $\rho$. For ViT, all $7!=5{,}040$ pairings are enumerated. For ResNet and DistilBERT, 199,999 random permutations are evaluated per method, with fixed seeds 42--45 and a plus-one correction. Table~\ref{tab:cross-bit-rank-stability} reports these permutation $p$-values; they are unadjusted across the six correlation tests and are distinct from the per-unit McNemar FDR values.

DistilBERT codebook is the only tested case with a significant positive association ($\rho=0.693$, permutation $p=0.00551$). It also remains below 0.05 with a six-test Bonferroni correction ($p=0.03306$). A post-hoc robustness analysis removes the prediction-disagreement tie-break and correlates the five distinct accuracy-degradation levels using average ranks. The association becomes slightly stronger ($\rho=0.713$, permutation $p=0.003765$), so the reported result is not created by disagreement tie-breaking. Disagreement alone is also associated across bits ($\rho=0.609$, permutation $p=0.01857$), but it is not the sole source of the accuracy association. Accuracy-only leave-one-tensor-out estimates remain positive ($\rho=0.645$--$0.766$; unadjusted permutation $p=0.002565$--$0.01655$). These checks address ties and single-unit influence, but remain exploratory on 15 tensors from one checkpoint. Most cases provide insufficient evidence of cross-bit association; nonsignificance, especially for the small transformer subsets, does not prove that no relationship exists. W8 sensitivity should therefore be validated at W4 before being used to guide low-bit selection.

\begin{table}[!t]
\centering
\caption{W4/W8 rank stability over matched units. Two-sided permutation $p$-values; exact for ViT and Monte Carlo otherwise.}
...
DistilBERT & Codebook & 15 & 0.693 & 0.00551 \\
...
The comparison concerns three selected checkpoints, not architecture alone. ViT's backbone remained frozen while its CIFAR-10 head was trained, whereas ResNet was adapted end-to-end; DistilBERT was fine-tuned on a different task. Pretraining, adaptation and checkpoint quality are not controlled across models. The 90.47\% ResNet baseline describes the checkpoint used; it does not establish whether inadequate adaptation caused quantization fragility. No matched-training control or additional checkpoint study was performed.

The severe ResNet W4 degradation remains unexplained. Frozen BatchNorm statistics, checkpoint properties and other pipeline effects are possible contributors, but their roles were not isolated experimentally. No recovery claim is made for BatchNorm recalibration, bias correction, equalization or adaptive rounding. The result is an observation of the evaluated pipeline and must not be generalized to all ResNets or CNNs.

Method comparisons also confound grid placement, granularity and level count: PTQ is uniform per-channel with $2^b-1$ levels, whereas codebooks are non-uniform per-tensor with $2^b$ centroids. Consequently, a method advantage cannot be attributed solely to non-uniform centroids. The transformer W4 isolated studies cover predefined architectural subsets rather than all eligible units; their rank conclusions apply to those subsets. Small accuracy changes, ties, exact-test resolution and limited sample sizes constrain both the sensitivity and cross-bit evidence, especially on the 872-example SST-2 split. Failure to reject a McNemar null is not an equivalence result.

The custom evaluation simulates quantized weights through FP32 reconstruction and FP32 activation arithmetic. Storage estimates describe packed parameter representations and do not establish latency, energy, throughput or runtime-memory improvements. Activation quantization and the transformer-specific calibration and inference methods discussed in the literature were not evaluated.

\section{Conclusion}

This study compared symmetric per-channel PTQ and tensor-local scalar codebooks for the selected ResNet-18, DistilBERT and ViT-B/16 checkpoints. Across the measured W8 configurations, the largest accuracy loss was 0.59 percentage points and estimated parameter-storage reduction was approximately 75\%. At W4, DistilBERT and ViT remained within 0.6 percentage points of their FP32 baselines at about $7.9\times$ compression, whereas ResNet fell from 90.47\% to 11.56\% with PTQ and 8.43\% with codebook quantization. These are checkpoint- and pipeline-specific observations. Different adaptation procedures and the unresolved cause of the ResNet collapse prevent attributing the contrast solely to architecture.

Codebook quantization achieved higher accuracy than PTQ for DistilBERT at W2 and ViT at W3. The ViT W3 difference was 57.55 percentage points. Centroid adaptation to the weight distribution is a possible explanation, but it was not tested independently of per-tensor versus per-channel granularity or eight versus seven reconstruction levels. The result supports a comparison of the evaluated configurations, not a causal claim about grid shape.

The isolated studies identified ResNet's first convolution as particularly vulnerable at W4 and measured smaller absolute changes in the sampled transformer units. They do not resolve the full-model failure mechanism. Most matched-unit comparisons did not provide significant evidence of cross-bit rank association, so general W8-to-W4 predictability remains unestablished by this design. The study documents accuracy and estimated storage trade-offs under explicit policies; claims about architecture-wide robustness, recovery methods or deployment efficiency require further evidence.
...
\bibitem{choi2017}
Y. Choi, M. El-Khamy, and J. Lee, ``Towards the limit of network quantization,'' in \emph{Proc. International Conf. Learning Representations (ICLR)}, 2017.
...
H. A. Abushahla, D. Varam, A. J. N. Panopio, and M. I. AlHajri, ``Quantized neural networks for microcontrollers: A comprehensive review of methods, platforms, and applications,'' arXiv:2508.15008v3, 2025.

\bibitem{fagerland2013}
M.~W. Fagerland, S. Lydersen, and P. Laake, ``The McNemar test for binary matched-pairs data: Mid-p and asymptotic are better than exact conditional,'' \emph{BMC Medical Research Methodology}, vol.~13, art.~91, 2013, doi: 10.1186/1471-2288-13-91.
...
\bibitem{xiao2022}
G. Xiao, J. Lin, M. Seznec, H. Wu, J. Demouth, and S. Han, ``SmoothQuant: Accurate and efficient post-training quantization for large language models,'' arXiv:2211.10438, 2022.

\bibitem{dettmers2022}
T. Dettmers, M. Lewis, Y. Belkada, and L. Zettlemoyer, ``LLM.int8(): 8-bit matrix multiplication for transformers at scale,'' in \emph{Proc. Advances in Neural Information Processing Systems (NeurIPS)}, 2022.

\bibitem{yuan2021}
Z. Yuan, C. Xue, Y. Chen, Q. Wu, and G. Sun, ``PTQ4ViT: Post-training quantization for vision transformers with twin uniform quantization,'' arXiv:2111.12293, 2021.
