...
\centering
\vspace{0pt}
\includegraphics[width=.55\linewidth]{resnet18_w4_w8_layer_sensitivity.png}
\refstepcounter{figure}\label{fig:resnet-cross-bit-sensitivity}
{\footnotesize Fig.~\thefigure. Matched ResNet-18 isolated kernel sensitivity at W4 and W8 for PTQ and codebook quantization. Only one kernel is quantized in each evaluation.\par}
\end{minipage}
\vspace{4pt}
\end{minipage}%
}]

For an \emph{isolated sensitivity} run, the experiment starts from the original FP32 checkpoint, quantizes exactly one analysis unit, leaves every other parameter in FP32, and evaluates the resulting model on the complete labelled evaluation split. ResNet uses individual eligible kernels, DistilBERT uses individual eligible embedding or kernel tensors, and ViT groups its 76 eligible tensors into 14 logical modules. The accuracy degradation for unit $u$ is $\Delta A_u=A_{\mathrm{FP32}}-A_u$, where $A_u$ is the accuracy when only unit $u$ is quantized. A negative value means that the configuration happens to classify more evaluation examples correctly; it is not treated as evidence of improved generalization.

All configurations use the complete evaluation split. ResNet is exhaustive at W8 and W4 (all 21 kernels). DistilBERT covers all 40 tensors at W8 and a predefined architecture-stratified subset of 15 at W4; ViT covers all 14 modules at W8 and a predefined stratified subset of seven at W4. The subsets were fixed before inspecting W4 outcomes.
